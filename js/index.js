...
function bindShowTilesParts() {
    ...
    viewer.mars.on(mars3d.event.click, function (event) {
        ...
        if (pickedObject && Cesium.defined(pickedObject.primitive) &&
            pickedObject.primitive.mars && pickedObject.primitive.mars.options.scenetree) {
            ...
            if (mars3d.widget.isActivate(tilesParts)) {
                ...
                if (parts.config.layerCfg == pickedObject.primitive.mars.options)
                    return;//当前已激活,并且单击了相同模型时跳出
            }

            mars3d.widget.activate({
                name: pickedObject.primitive.mars.options.name + " 构件",
                uri: tilesParts,
                layerCfg: pickedObject.primitive.mars.options,
                disableOther: false
            });
        }
    });
